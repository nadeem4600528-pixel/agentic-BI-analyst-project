...
import { analyze, dashboard, getApiError, profileUpload, reportExport, transform, workflows } from './api'
...
export default function App() {
  ...
  const upload = e => { const file = e.target.files?.[0]; if (!file) return; setFileName(file.name); run(() => profileUpload(file), data => { const rows = data.data || data.records || data.preview; if (Array.isArray(rows) && rows.length) { setRecords(rows); setColumns(Object.keys(rows[0])) } else { setRecords([]); setColumns([]); setError('The uploaded file contains no data rows.') } }) }
