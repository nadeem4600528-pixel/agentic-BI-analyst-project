...
export default function App() {
  ...
  const upload = e => { const file = e.target.files?.[0]; if (!file) return; setFileName(file.name); run(() => profileUpload(file), data => applyRows(data.data || data.records || data.preview, file.name)) }
  const loadDemo = async () => { setLoading(true); setError(''); try { const res = await demoDataset(); const d = res.data; applyRows(d.data, d.filename || 'Sample sales dataset'); setDateColumn(d.suggested?.date_column || ''); setValueColumn(d.suggested?.value_column || ''); setCategoryColumn(d.suggested?.category_column || ''); setPage('dashboard') } catch (e) { setError(await getApiError(e, 'Could not load demo data')) } finally { setLoading(false) } }
  ...
  const content = page === 'dashboard' ? <Dashboard records={records} payload={payload} run={run} result={result} /> : page === 'upload' ? <Upload fileName={fileName} upload={upload} records={records} loadDemo={loadDemo} /> : page === 'transform' ? <Transform records={records} columns={columns} run={run} setRecords={setRecords} setColumns={setColumns} /> : page === 'reports' ? <Reports payload={payload} download={download} run={run} result={result} /> : page === 'jobs' ? <Jobs run={run} result={result} /> : <DataView title={title} records={records} result={result} action={() => run(page === 'analysis' ? () => analyze(payload) : () => dashboard(payload))} actionLabel={page === 'analysis' ? 'Run analysis' : 'Scan dataset'} />
